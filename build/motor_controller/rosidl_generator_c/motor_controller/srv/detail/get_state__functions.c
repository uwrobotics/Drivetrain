// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from motor_controller:srv/GetState.idl
// generated code does not contain a copyright notice
#include "motor_controller/srv/detail/get_state__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

bool
motor_controller__srv__GetState_Request__init(motor_controller__srv__GetState_Request * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
motor_controller__srv__GetState_Request__fini(motor_controller__srv__GetState_Request * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
motor_controller__srv__GetState_Request__are_equal(const motor_controller__srv__GetState_Request * lhs, const motor_controller__srv__GetState_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
motor_controller__srv__GetState_Request__copy(
  const motor_controller__srv__GetState_Request * input,
  motor_controller__srv__GetState_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

motor_controller__srv__GetState_Request *
motor_controller__srv__GetState_Request__create()
{
  motor_controller__srv__GetState_Request * msg = (motor_controller__srv__GetState_Request *)malloc(sizeof(motor_controller__srv__GetState_Request));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(motor_controller__srv__GetState_Request));
  bool success = motor_controller__srv__GetState_Request__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
motor_controller__srv__GetState_Request__destroy(motor_controller__srv__GetState_Request * msg)
{
  if (msg) {
    motor_controller__srv__GetState_Request__fini(msg);
  }
  free(msg);
}


bool
motor_controller__srv__GetState_Request__Sequence__init(motor_controller__srv__GetState_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  motor_controller__srv__GetState_Request * data = NULL;
  if (size) {
    data = (motor_controller__srv__GetState_Request *)calloc(size, sizeof(motor_controller__srv__GetState_Request));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = motor_controller__srv__GetState_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        motor_controller__srv__GetState_Request__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
motor_controller__srv__GetState_Request__Sequence__fini(motor_controller__srv__GetState_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      motor_controller__srv__GetState_Request__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

motor_controller__srv__GetState_Request__Sequence *
motor_controller__srv__GetState_Request__Sequence__create(size_t size)
{
  motor_controller__srv__GetState_Request__Sequence * array = (motor_controller__srv__GetState_Request__Sequence *)malloc(sizeof(motor_controller__srv__GetState_Request__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = motor_controller__srv__GetState_Request__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
motor_controller__srv__GetState_Request__Sequence__destroy(motor_controller__srv__GetState_Request__Sequence * array)
{
  if (array) {
    motor_controller__srv__GetState_Request__Sequence__fini(array);
  }
  free(array);
}

bool
motor_controller__srv__GetState_Request__Sequence__are_equal(const motor_controller__srv__GetState_Request__Sequence * lhs, const motor_controller__srv__GetState_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!motor_controller__srv__GetState_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
motor_controller__srv__GetState_Request__Sequence__copy(
  const motor_controller__srv__GetState_Request__Sequence * input,
  motor_controller__srv__GetState_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(motor_controller__srv__GetState_Request);
    motor_controller__srv__GetState_Request * data =
      (motor_controller__srv__GetState_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!motor_controller__srv__GetState_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          motor_controller__srv__GetState_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!motor_controller__srv__GetState_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `current_state`
#include "motor_controller/msg/detail/state__functions.h"

bool
motor_controller__srv__GetState_Response__init(motor_controller__srv__GetState_Response * msg)
{
  if (!msg) {
    return false;
  }
  // current_state
  if (!motor_controller__msg__State__init(&msg->current_state)) {
    motor_controller__srv__GetState_Response__fini(msg);
    return false;
  }
  return true;
}

void
motor_controller__srv__GetState_Response__fini(motor_controller__srv__GetState_Response * msg)
{
  if (!msg) {
    return;
  }
  // current_state
  motor_controller__msg__State__fini(&msg->current_state);
}

bool
motor_controller__srv__GetState_Response__are_equal(const motor_controller__srv__GetState_Response * lhs, const motor_controller__srv__GetState_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_state
  if (!motor_controller__msg__State__are_equal(
      &(lhs->current_state), &(rhs->current_state)))
  {
    return false;
  }
  return true;
}

bool
motor_controller__srv__GetState_Response__copy(
  const motor_controller__srv__GetState_Response * input,
  motor_controller__srv__GetState_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // current_state
  if (!motor_controller__msg__State__copy(
      &(input->current_state), &(output->current_state)))
  {
    return false;
  }
  return true;
}

motor_controller__srv__GetState_Response *
motor_controller__srv__GetState_Response__create()
{
  motor_controller__srv__GetState_Response * msg = (motor_controller__srv__GetState_Response *)malloc(sizeof(motor_controller__srv__GetState_Response));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(motor_controller__srv__GetState_Response));
  bool success = motor_controller__srv__GetState_Response__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
motor_controller__srv__GetState_Response__destroy(motor_controller__srv__GetState_Response * msg)
{
  if (msg) {
    motor_controller__srv__GetState_Response__fini(msg);
  }
  free(msg);
}


bool
motor_controller__srv__GetState_Response__Sequence__init(motor_controller__srv__GetState_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  motor_controller__srv__GetState_Response * data = NULL;
  if (size) {
    data = (motor_controller__srv__GetState_Response *)calloc(size, sizeof(motor_controller__srv__GetState_Response));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = motor_controller__srv__GetState_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        motor_controller__srv__GetState_Response__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
motor_controller__srv__GetState_Response__Sequence__fini(motor_controller__srv__GetState_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      motor_controller__srv__GetState_Response__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

motor_controller__srv__GetState_Response__Sequence *
motor_controller__srv__GetState_Response__Sequence__create(size_t size)
{
  motor_controller__srv__GetState_Response__Sequence * array = (motor_controller__srv__GetState_Response__Sequence *)malloc(sizeof(motor_controller__srv__GetState_Response__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = motor_controller__srv__GetState_Response__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
motor_controller__srv__GetState_Response__Sequence__destroy(motor_controller__srv__GetState_Response__Sequence * array)
{
  if (array) {
    motor_controller__srv__GetState_Response__Sequence__fini(array);
  }
  free(array);
}

bool
motor_controller__srv__GetState_Response__Sequence__are_equal(const motor_controller__srv__GetState_Response__Sequence * lhs, const motor_controller__srv__GetState_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!motor_controller__srv__GetState_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
motor_controller__srv__GetState_Response__Sequence__copy(
  const motor_controller__srv__GetState_Response__Sequence * input,
  motor_controller__srv__GetState_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(motor_controller__srv__GetState_Response);
    motor_controller__srv__GetState_Response * data =
      (motor_controller__srv__GetState_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!motor_controller__srv__GetState_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          motor_controller__srv__GetState_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!motor_controller__srv__GetState_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
